...
void serial_debug(char* fmt, ...)
{
    ...
    int len = strlen(tmpLog);

    int i = 0;
    for (; i < len; i++)
    {
        usart_send_blocking(USART1, (uint16_t)tmpLog[i]);
    }

    usart_send_blocking(USART1, (uint16_t)'\n');
}
...
int main(void)
{
    ...
    hci_uart_config_t  * hci_config = hci_uart_config_csr_instance();
    remote_device_db_t * remote_db = (remote_device_db_t *) &remote_device_db_memory;
    hci_init(transport, hci_config, control, remote_db);
